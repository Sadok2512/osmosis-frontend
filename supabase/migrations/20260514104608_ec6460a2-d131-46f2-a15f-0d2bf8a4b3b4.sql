...
CREATE OR REPLACE FUNCTION public.topo_perimeter_count(p_filters jsonb DEFAULT '[]'::jsonb, p_logic text DEFAULT 'AND')
RETURNS jsonb
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
BEGIN
  ...
  FOR flt IN SELECT * FROM jsonb_array_elements(p_filters) LOOP
    ...
    IF col = ANY(allowed_cols) AND jsonb_typeof(flt->'values') = 'array' AND jsonb_array_length(flt->'values') > 0 THEN
      SELECT string_agg(quote_literal(v), ',') INTO vals_text
      FROM jsonb_array_elements_text(flt->'values') AS v;
      where_parts := array_append(where_parts, format('%I IN (%s)', col, vals_text));
    END IF;
  END LOOP;
