...
BEGIN
  ...
  FOR flt IN SELECT * FROM jsonb_array_elements(p_filters) LOOP
    ...
    IF col = ANY(allowed_cols) AND jsonb_typeof(flt->'values') = 'array' AND jsonb_array_length(flt->'values') > 0 THEN
      per_value_parts := ARRAY[]::text[];
      FOR v IN SELECT jsonb_array_elements_text(flt->'values') LOOP
        -- Case-insensitive exact OR substring match (so "marseille" matches "MARSEILLE_CENTRE" etc.)
        per_value_parts := array_append(
          per_value_parts,
          format('(lower(%I::text) = lower(%L) OR %I::text ILIKE %L)', col, v, col, '%' || v || '%')
        );
      END LOOP;
      where_parts := array_append(where_parts, '(' || array_to_string(per_value_parts, ' OR ') || ')');
    END IF;
  END LOOP;
