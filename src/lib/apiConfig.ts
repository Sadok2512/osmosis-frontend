...
export function getApiUrl(functionName: string): string {
  ...
  if (source === 'vps') {
    ...
    const parserPrefixes = ['topo', 'config/topo', 'qoe-map', 'qoe-metrics', 'dump-parameter', 'parameter-changes', 'bi-query', 'bi-distinct', 'bi-date-range', 'bi-catalog', 'sentinel', 'alarms', 'cm', 'pm', 'neighbors', 'filters', 'agent'];
