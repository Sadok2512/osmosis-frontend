          ...
          >
            ...
            <div className="flex items-center gap-2 mb-3 relative z-10">
              ...
              <Popover>
                ...
                <PopoverContent className="w-[300px] p-0 z-50 overflow-hidden" align="end" side="bottom">
                  ...
                  <div className="p-3 space-y-3 max-h-[420px] overflow-y-auto">
                    ...
                    <div className="space-y-1.5">
                      ...
                      {defs.map((d, i) => {
                        const kId = kpiIds[i];
                        return (
                          <div key={kId} className="flex items-center justify-between gap-1 py-1 border-b border-border/20 last:border-0">
                            <div className="flex items-center gap-1.5 min-w-0">
                              <span className="w-2 h-2 rounded-full shrink-0" style={{ backgroundColor: d.color }} />
                              <span className="text-[10px] font-medium text-foreground truncate max-w-[120px]">{d.label}</span>
                            </div>
                            <div className="flex items-center gap-1 shrink-0">
                              <div className="flex items-center bg-muted/50 rounded border border-border/40 overflow-hidden">
                                <button
                                  ...
                                  className={cn('px-1.5 py-0.5 text-[8px] font-bold transition-colors', (cfg.yAxisAssignments?.[kId] || 0) === 0 ? 'bg-primary/20 text-primary' : 'text-muted-foreground hover:text-foreground')}
                                >L</button>
                                <button
                                  onClick={() => onUpdateSlotConfig(slot.id, { yAxisAssignments: { ...cfg.yAxisAssignments, [kId]: 1 } })}
                                  className={cn('px-1.5 py-0.5 text-[8px] font-bold transition-colors', cfg.yAxisAssignments?.[kId] === 1 ? 'bg-primary/20 text-primary' : 'text-muted-foreground hover:text-foreground')}
                                >R</button>
                              </div>
                              <button onClick={() => onChangeSlotKpi(slot.id, kId)} className="text-muted-foreground hover:text-destructive">
                                <X className="w-3 h-3" />
                              </button>
                            </div>
                          </div>
                      ...
                      <Button variant="outline" size="sm" className="h-6 text-[10px] px-2 w-full mt-1" onClick={() => onOpenKpiSelector(slot.id)}>
                        ...
                        {counterIds.map((cId, ci) => {
                          ...
                          return (
                            <div key={cId} className="flex items-center justify-between gap-1 py-1 border-b border-border/20 last:border-0">
                              ...
                              <button onClick={() => onSetSlotCounterIds(slot.id, counterIds.filter(c => c !== cId))} className="text-muted-foreground hover:text-destructive">
                                <X className="w-3 h-3" />
                              </button>
                            </div>
                          );
                    ...
                    <Button variant="outline" size="sm" className="h-6 text-[10px] px-2 w-full" onClick={() => setCounterSelectorSlotId(slot.id)}>
                      ...
                      <div className="grid grid-cols-4 gap-1">
                        {CHART_TYPES.map(ct => (
                          <button
                            ...
                            className={cn(
                              'flex flex-col items-center gap-0.5 px-1.5 py-1.5 rounded-md text-[9px] font-semibold transition-all border',
                              cfg.chartType === ct.value ? 'border-primary/50 bg-primary/10 text-primary' : 'border-border/30 text-muted-foreground hover:bg-muted/50'
                            )}
                          >
                    ...
                    {/* ── Style ── */}
                    <div className="space-y-2">
                      <span className="text-[9px] font-bold text-muted-foreground uppercase tracking-wider">Style</span>
                      <div className="grid grid-cols-2 gap-x-4 gap-y-1.5">
                        ...
                        <div className="flex items-center justify-between">
                          <span className="text-[10px] text-foreground">Grid</span>
                          <Switch checked={cfg.showGrid} onCheckedChange={v => onUpdateSlotConfig(slot.id, { showGrid: v })} className="scale-[0.65]" />
                        </div>
                        <div className="flex items-center justify-between">
                          <span className="text-[10px] text-foreground">Thresholds</span>
                          <Switch checked={cfg.showThresholds} onCheckedChange={v => onUpdateSlotConfig(slot.id, { showThresholds: v })} className="scale-[0.65]" />
                        </div>
                      </div>
                    ...
                    <div className="space-y-1.5">
                      <div className="flex items-center justify-between">
                        <span className="text-[9px] font-bold text-muted-foreground uppercase tracking-wider">Y-Axis</span>
                        <div className="flex items-center bg-muted/50 rounded border border-border/40 overflow-hidden">
                          {(['L', 'R'] as const).map(side => {
                            const isActiveAxis = (cfg as any).__activeYTab === side || (!(cfg as any).__activeYTab && side === 'L');
                            return (
                              <button key={side} onClick={() => onUpdateSlotConfig(slot.id, { __activeYTab: side } as any)}
                                className={cn('px-2.5 py-0.5 text-[9px] font-bold transition-colors', isActiveAxis ? 'bg-primary/20 text-primary' : 'text-muted-foreground hover:text-foreground')}
                              >{side}</button>
                            );
                          })}
                        </div>
                          ...
                          <>
                            <div className="flex gap-1">
                              {(['auto', 'manual'] as const).map(mode => (
                                <button key={mode} onClick={() => onUpdateSlotConfig(slot.id, { [axisKey]: { ...axisCfg, mode } })}
