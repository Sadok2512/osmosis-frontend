...
/**
 * Fetch the KPI catalog from the VPS backend (same source as
 * the legacy "Référentiel KPI Réseau" page in DocumentationPage).
 * Maps the raw VPS payload into the unified KpiCatalogEntry shape
 * used by the KPI Reference workspaces.
 */
export async function fetchKpiCatalogFromVps(): Promise<KpiCatalogEntry[]> {
  ...
  if (!res.ok) {
    throw new Error(`VPS catalog fetch failed: ${res.status}`);
  }
  const data = await res.json();
  if (!Array.isArray(data)) return [];

  return data.map((k: any): KpiCatalogEntry => {
    ...
    return {
      kpi_id: String(k.kpi_key || k.kpi_code || ''),
