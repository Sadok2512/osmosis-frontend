...
const SmallInput: React.FC<React.InputHTMLAttributes<HTMLInputElement>> = ({ className, ...props }) => (
  <input
    {...props}
    className={cn(
      'h-[30px] px-2 rounded-lg border border-border/50 bg-background text-[11px] text-foreground',
...
const AxesPopover: React.FC<AxesPopoverProps> = ({ axisConfig: ext, onAxisConfigChange }) => {
      ...
      <PopoverContent className="w-[240px] p-4" align="start" sideOffset={8}>
        <div className="space-y-2.5">
          ...
          <FieldRow label="Titre">
            <SmallInput value={axis.yTitle} onChange={e => set({ yTitle: e.target.value })} placeholder="" className="w-[80px]" />
          </FieldRow>
          <FieldRow label="Min">
            <SmallInput type="number" value={axis.yMin === 'auto' ? '' : String(axis.yMin)} placeholder="Auto"
              onChange={e => set({ yMin: e.target.value === '' ? 'auto' : Number(e.target.value) })} className="w-[72px]" />
          </FieldRow>
          <FieldRow label="Max">
            <SmallInput type="number" value={axis.yMax === 'auto' ? '' : String(axis.yMax)} placeholder="Auto"
              onChange={e => set({ yMax: e.target.value === '' ? 'auto' : Number(e.target.value) })} className="w-[72px]" />
          </FieldRow>
          ...
          <FieldRow label="Décimales">
            ...
            ]} onChange={v => set({ yDecimals: Number(v) })} />
          </FieldRow>
          ...
          <div className="pt-2 border-t border-border/30">
            ...
            <FieldRow label="Format">
              ...
              ]} onChange={v => set({ xFormat: v as any })} />
            </FieldRow>
            <div className="flex items-center justify-between min-h-[32px]">
