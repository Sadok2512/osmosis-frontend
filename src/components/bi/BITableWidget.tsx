...
import { GripVertical, Trash2, Plus, X, Table2, Settings } from 'lucide-react';
import { BI_KPIS, BIKPI, KPI_UNITS, BI_DIMENSIONS, BIDimension } from './biTypes';
...
const BITableWidget: React.FC<Props> = ({ config, onChange, onDelete }) => {
  ...
  const toggleKpi = (kpi: BIKPI) => {
    const kpis = config.kpis.includes(kpi)
      ? config.kpis.filter(k => k !== kpi)
      : [...config.kpis, kpi];
  ...
  return (
    <div className="w-full h-full flex flex-col bg-card rounded-xl border border-border overflow-hidden group">
      ...
      <div className="flex items-center gap-2 px-3 py-2 border-b border-border bg-muted/30 shrink-0">
        ...
        />
        <button onClick={() => setShowKpiPicker(!showKpiPicker)}
          className="p-1 rounded hover:bg-muted text-muted-foreground hover:text-primary transition-colors" title="Add/Remove KPIs">
      ...
      {/* KPI picker dropdown */}
      {showKpiPicker && (
        <div className="border-b border-border bg-muted/20 p-2 max-h-[200px] overflow-auto">
          <div className="flex items-center justify-between mb-2">
            <span className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">Select KPIs</span>
            <button onClick={() => setShowKpiPicker(false)} className="p-0.5 rounded hover:bg-muted">
      ...
      {showSettings && (
        <div className="border-b border-border bg-muted/20 p-3 space-y-2">
          <div className="flex items-center gap-3">
            <label className="text-[10px] font-semibold text-muted-foreground uppercase">Dimension</label>
            <select value={config.dimension} onChange={e => onChange({ ...config, dimension: e.target.value as BIDimension })}
              className="flex-1 text-[10px] bg-background border border-border rounded px-2 py-1 text-foreground outline-none">
              {BI_DIMENSIONS.map(d => <option key={d} value={d}>{d}</option>)}
            </select>
          </div>
          <div className="flex items-center gap-4">
            <label className="flex items-center gap-1.5 text-[10px] text-muted-foreground cursor-pointer">
              <input type="checkbox" checked={config.striped} onChange={e => onChange({ ...config, striped: e.target.checked })} className="rounded" />
              Striped
            </label>
            <label className="flex items-center gap-1.5 text-[10px] text-muted-foreground cursor-pointer">
              <input type="checkbox" checked={config.compact} onChange={e => onChange({ ...config, compact: e.target.checked })} className="rounded" />
              Compact
            </label>
          </div>
        </div>
                ...
                <tr>
                  ...
                  {config.kpis.map(kpi => (
                    <th key={kpi} className={`${config.compact ? 'px-2 py-1' : 'px-3 py-2'} font-bold text-foreground border-b border-border text-right`}>
                      <span className="whitespace-nowrap">{kpi.replace(/_/g, ' ')}</span>
