// ── Sentinel API Service — calls Agent Layer through VPS proxy ──
...
import { isLocalMode, getVpsProxyUrl, getVpsProxyHeaders } from '@/lib/apiConfig';

const getLocalBase = () => import.meta.env.VITE_SENTINEL_API_URL || 'http://localhost:1000';
...
async function fetchJson<T>(path: string): Promise<T> {
  ...
  try {
    ...
    if (isLocalMode()) {
      fetchUrl = `${getLocalBase()}${path}`;
      headers = { 'x-api-key': 'agent_secret_key' };
    } else {
      // Route through VPS proxy
      fetchUrl = getVpsProxyUrl('agent', path);
      headers = {
        ...getVpsProxyHeaders(),
...
export async function fetchDates(): Promise<string[]> {
  const raw = await fetchJson<any>('/api/dates');
...
export async function fetchOverview(date: string): Promise<DashboardOverviewData> {
  const raw = await fetchJson<any>(`/api/dashboard/overview?date=${date}`);
...
export async function fetchAnomalies(filters: AnomalyFilters): Promise<Anomaly[]> {
  ...
  const raw = await fetchJson<any>(`/api/anomalies?${params}`);
...
export async function fetchAnomalySummary(date: string): Promise<AnomalySummary[]> {
  const raw = await fetchJson<any>(`/api/anomalies/summary?date=${date}`);
  if (Array.isArray(raw)) return raw;
...
export async function fetchKPIHistory(
  ...
  return fetchJson<KPIHistoryData>(
    `/api/kpi/history?dimension_1=${encodeURIComponent(dimension_1)}&dimension_2=${encodeURIComponent(dimension_2)}&kpi=${kpi}&days=${days}`
  );
}
...
export async function fetchKPICompare(dimension_2: string, date: string): Promise<KPICompareData> {
  return fetchJson<KPICompareData>(
    `/api/kpi/compare?dimension_2=${encodeURIComponent(dimension_2)}&date=${date}`
  );
}

export async function fetchClusters(date: string, dimension: SentinelDimension = 'Cellule'): Promise<ClusterData[]> {
  const raw = await fetchJson<any>(`/api/clusters?date=${date}&dimension=${dimension}`);
  if (Array.isArray(raw)) return raw;
...
export async function fetchClusterMembers(
  ...
  const raw = await fetchJson<any>(
    `/api/clusters/${clusterId}/members?date=${date}&dimension=${dimension}`
  );
  if (Array.isArray(raw)) return raw;
...
export async function fetchDimensionValues(
  ...
  return fetchJson<string[]>(`/api/dimensions/values?${params}`);
}
