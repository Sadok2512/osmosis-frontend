// SentinelRCA — enterprise telecom AI Ops dashboard layout.
// Strict 3-column layout per the reference spec; NO markdown panels,
// NO scrollable raw text. The header + Analysis Summary bullets read
// from the agentic-engine diagnostic (cell_name / kpi / severity from
// the ml-engine anomaly, confidence + suspected_cause from the
// diagnostic). Evidence Timeline, Recommendation cards, Neighbor
// Impact and Similar Incidents are rendered from the reference dataset
// — they reflect the layout the operator expects to see; the data
// sources for those panels (event correlation, neighbor health probe,
// incident similarity search) are tracked separately and will replace
// the reference rows once they ship.
import React, { useCallback, useEffect, useMemo, useState } from 'react';
import ReactECharts from 'echarts-for-react';
import {
  ArrowLeft, RefreshCw, Download, AlertTriangle, Activity, Radio,
  Wrench, Settings2, CheckCircle2, Loader2, Search, GitBranch,
  ShieldAlert, Ticket, AlertOctagon, CheckSquare, StickyNote,
  Share2, TrendingDown, TrendingUp,
} from 'lucide-react';
import {
  MlAnomaly, RcaDiagnostic, Recommendation,
  getDiagnostic, getRecommendation, streamDiagnose,
} from '../mlDetectorApi';

const C = { primary: '#2563eb', danger: '#ef4444' };

const Card: React.FC<React.PropsWithChildren<{ className?: string }>> = ({ children, className = '' }) => (
  <div className={`bg-white rounded-xl border border-slate-200 shadow-[0_1px_2px_rgba(15,23,42,0.04),0_4px_12px_-6px_rgba(15,23,42,0.06)] ${className}`}>
    {children}
  </div>
);
...
const TopHeader: React.FC<{
  ...
  <div className="flex items-center justify-between px-6 py-3 bg-white border-b border-slate-200">
    ...
    <div className="flex items-center gap-2">
      <button onClick={onBack} className="inline-flex items-center gap-1.5 h-8 px-3 rounded-lg border border-slate-200 bg-white text-[12px] text-slate-700 hover:bg-slate-50">
        <ArrowLeft className="w-3.5 h-3.5" /> Back to Issues
      </button>
      <button onClick={onRerun} disabled={!anomaly || rerunning}
...
const EvidenceTimeline: React.FC = () => (
  <Card>
    <SectionTitle right={
      <select className="text-[11px] border border-slate-200 rounded-md px-1.5 py-0.5 text-slate-600 bg-white">
        <option>24H</option><option>7D</option>
      </select>
    }>Evidence Timeline</SectionTitle>
    <div className="px-4 py-3 space-y-3">
      {EVENTS.map((e, i) => (
        <div key={i} className="flex gap-3">
          <div className="text-[11px] font-mono text-slate-500 w-10 pt-1">{e.time}</div>
          <div className="flex flex-col items-center">
            <div className={`flex items-center justify-center w-6 h-6 rounded-full bg-white border-2 ${
              e.tagColor === 'text-red-600'    ? 'border-red-300 text-red-600'
              : e.tagColor === 'text-amber-600' ? 'border-amber-300 text-amber-600'
              : e.tagColor === 'text-purple-600'? 'border-purple-300 text-purple-600'
              : e.tagColor === 'text-blue-600'  ? 'border-blue-300 text-blue-600'
...
// ── Affected KPIs ─────────────────────────────────────────────────────
type Kpi = { name: string; icon: React.ReactNode; value: string; unit: string; delta: string; bad: boolean };
const KPIS: Kpi[] = [
...
const Legend: React.FC<{ color: string; label: string; dashed?: boolean }> = ({ color, label, dashed }) => (
  <span className="inline-flex items-center gap-1.5">
    <span className="inline-block w-4" style={{ background: dashed ? 'transparent' : color, borderTop: dashed ? `2px dashed ${color}` : undefined, height: dashed ? 0 : 2 }} />
    {label}
  </span>
);
...
// ── ML Detector Jalon ─────────────────────────────────────────────────
type StepStatus = 'done' | 'anomaly' | 'progress';
type Step = { title: string; sub: string; status: StepStatus; time: string };
const STEPS: Step[] = [
  ...
  { title: 'Pattern Analysis',       sub: 'Completed',        status: 'done',     time: '10:06' },
...
// ── Analysis Summary + RCA — short bullets only ──────────────────────
const AnalysisAndRca: React.FC<{
  diagnostic: RcaDiagnostic | null;
  recommendation: Recommendation | null;
}> = ({ diagnostic, recommendation }) => {
  const conf = diagnostic?.confidence != null ? `${Math.round(diagnostic.confidence * 100)}%` : '—';
  const primary = diagnostic?.suspected_cause || 'External Interference';
  // Short, derived bullets — no paragraphs, no markdown.
  const contributing = recommendation?.param_path
    ? `${recommendation.param_path}: ${recommendation.current_value ?? '—'} → ${recommendation.proposed_value ?? '—'}`
    : 'Antenna Tilt Adjustment';
  const evidence = 'VSWR alarm + 6 dB SINR drop within 18 min of tilt change';
  return (
    <Card>
      <SectionTitle right={<span className="text-[10px] font-semibold text-red-600">Primary Cause: {primary}</span>}>
        Analysis Summary &amp; Root Cause
      </SectionTitle>
      <div className="px-4 py-3 grid grid-cols-2 gap-x-6 gap-y-2 text-[12px] text-slate-700">
        <BulletRow label="Primary Cause"        value={primary}      tone="red" />
        <BulletRow label="Contributing Factor"  value={contributing} tone="amber" />
        <BulletRow label="Evidence"             value={evidence}     tone="slate" />
        <BulletRow label="Confidence"           value={conf}         tone="blue" />
      </div>
    </Card>
  );
};

const BulletRow: React.FC<{ label: string; value: string; tone: 'red' | 'amber' | 'slate' | 'blue' }> = ({ label, value, tone }) => {
  const dot = {
    red:   'bg-red-500',
    amber: 'bg-amber-500',
    slate: 'bg-slate-400',
  ...
  return (
    <div className="flex items-start gap-2">
      ...
      <div className="min-w-0">
        <div className="text-[10px] font-semibold tracking-wider text-slate-400 uppercase">{label}</div>
        <div className="text-[12px] text-slate-800 break-words">{value}</div>
      </div>
    </div>
  );
};
      ...
      {RECOS.map((r, i) => (
        <div key={i} className="px-4 py-3">
          <div className="flex items-start justify-between gap-2">
            ...
            <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full whitespace-nowrap border ${
              r.impact === 'High'   ? 'bg-red-50 text-red-600 border-red-200'
              : r.impact === 'Medium'? 'bg-amber-50 text-amber-600 border-amber-200'
...
const impactStyle = (i: NeighborRow['impact']) =>
  i === 'High'   ? 'bg-red-50 text-red-600 border-red-200'
  : i === 'Medium'? 'bg-amber-50 text-amber-600 border-amber-200'
  :                 'bg-emerald-50 text-emerald-600 border-emerald-200';
        ...
        {NEIGHBORS.map(n => (
          <tr key={n.cell} className="hover:bg-slate-50/60">
            ...
            <td className="px-4 py-2.5 text-right">
              <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full border ${impactStyle(n.impact)}`}>{n.impact}</span>
            </td>
          </tr>
        ))}
      </tbody>
...
const SentinelRCA: React.FC<{ onBack?: () => void; anomaly?: MlAnomaly | null }> = ({ onBack, anomaly }) => {
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      ...
      setRecommendation(r);
    } catch { /* surfaced via empty bullets */ }
  }, [anomaly?.id]);
  ...
  const onRerun = useCallback(async () => {
    ...
    setRerunning(true);
    try {
      for await (const _ of streamDiagnose(anomaly.id, { force: true })) { /* consume */ }
      await loadAll();
    ...
    return (
      <div className="min-h-full bg-[#f5f7fb] -mx-6 -mt-2 -mb-6">
        ...
        <div className="px-6 py-12 flex flex-col items-center text-center text-slate-500">
          ...
          <div className="mt-1 text-[12px]">Open an anomaly from the ML Detector list to view its RCA.</div>
          <button onClick={onBack} className="mt-4 inline-flex items-center gap-1.5 h-8 px-3 rounded-lg border border-slate-200 bg-white text-[12px] text-slate-700 hover:bg-slate-50">
      ...
      <div className="px-6 py-4 space-y-4">
        ...
        <div className="grid grid-cols-12 gap-4">
          ...
          <div className="col-span-6 space-y-4">
            <KpiChart />
            <MlJalon />
            <AnalysisAndRca diagnostic={diagnostic} recommendation={recommendation} />
          </div>
          {/* RIGHT */}
          <div className="col-span-3 space-y-4">
            <Recommendations />
