...
import { getVpsProxyUrl, getVpsProxyHeaders, fetchVpsWithRetry } from '@/lib/apiConfig';
  ...
  useEffect(() => {
    ...
    async function extractFiltersFromSites() {
      try {
        ...
        const resp = await fetchVpsWithRetry(url, { headers: getVpsProxyHeaders() }, { maxRetries: 3, timeoutMs: 45_000 });
