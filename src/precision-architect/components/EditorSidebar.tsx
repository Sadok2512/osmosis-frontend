...
export default function EditorSidebar({ onClose }: EditorSidebarProps) {
  ...
  const buildTheme = (d: DraftConfig): DashboardTheme => {
    ...
    return {
      ...
      photoOffsetX: activeTheme.photoOffsetX ?? DEFAULT_DASHBOARD_THEME.photoOffsetX,
      photoOffsetY: activeTheme.photoOffsetY ?? DEFAULT_DASHBOARD_THEME.photoOffsetY,
      reportInfo: {
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-3">
        ...
        <CollapsibleSection title="Report Info" icon={PanelTop} open={openSections.reportInfo} onToggle={() => toggle('reportInfo')}>
          ...
          <div className={cn('space-y-1 pt-1 transition-opacity', !draft.reportInfoShow && 'opacity-50 pointer-events-none')}>
            ...
            <ToggleField label="Date" value={draft.reportInfoDate} onChange={(v) => updateReportInfo('reportInfoDate', v)} />
            <ToggleField label="Granularity" value={draft.reportInfoGranularity} onChange={(v) => updateReportInfo('reportInfoGranularity', v)} />
            <ToggleField label="Filters" value={draft.reportInfoFilters} onChange={(v) => updateReportInfo('reportInfoFilters', v)} />
          </div>
        </CollapsibleSection>
